...
customMatchers.toBeAFunctionalComponent = function(options) {
  if (isAComponent(options) && options.functional === true) {
    return {
      message: () =>
        `expected ${this.utils.printReceived(
          options
        )} not to be a functional Vue component`,
      pass: true,
    }
  } else {
    return {
      message: () =>
        `expected ${this.utils.printReceived(
          options
        )} to be a valid functional Vue component, exported from a .vue file`,
      pass: false,
    }
  }
}
